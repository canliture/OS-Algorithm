...
void Produce()
{
	std::cout << std::endl << "Producing " << ++ProductID << " ... ";                  //仅仅生产
...
{
	while (p_ccontinue) {
		WaitForSingleObject(EmptySemaphore, INFINITE);        //p(empty);          //如果有空闲空间可以填充
		  WaitForSingleObject(Mutex, INFINITE);    //p(mutex);                     //临界区互斥
		    Produce();
		    Append();
		    Sleep(1500);                                                          
...
{
	while (p_ccontinue) {
		WaitForSingleObject(FullSemaphore, INFINITE); //P(full);               //如果仍然有空间可以消费  (如果Full信号不为)
		  WaitForSingleObject(Mutex, INFINITE);  //P(mutex);                   //临界区互斥
		    Take();
		    Consume();
		    Sleep(1500);
